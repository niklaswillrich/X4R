...
void mdDataSetGetNames(Rcpp::CharacterVector &names, ns4__Axes *axes, int i, bool isRow, std::string separator)
{
  int axis;
  if (isRow)
    axis = 1;
  ...
  for (unsigned j = 0; j < memberList.size(); j++) {
    name = name + *memberList[j]->Caption + separator;
  }
  names.push_back(name.substr(0, name.size() - separator.size()));
}
...
RcppExport SEXP RXMLAExecute(SEXP handle, SEXP query, SEXP rPropertiesString, SEXP separatorString)
{
  ...
  std::string separator = CHAR(STRING_ELT(separatorString,0));
  command.Statement = &statement;
  execute.Command = &command;
  execute.Properties = &properties;
  properties.PropertyList = &propertyList;
  ...
  if (service.Execute(connectionString, NULL, &execute, &response) == SOAP_OK) {
    ...
    if (response.return_->ns4__root != NULL && response.return_->ns4__root->__union_ResultXmlRoot != NULL && response.return_->ns4__root->__union_ResultXmlRoot->Axes != NULL) {
      ...
      for (int row = 0; row < numRows; row++)	{
        ...
        mdDataSetGetNames(rowNames, axes, row, true, separator );
      }
      
      for (int col = 0; col < numCols; col++)	{
        mdDataSetGetNames(colNames, axes, col, false, separator);
      }
